...
resultMatrix?.onRowSelection()
        ?.triggerEvent(api.dashboardWideEvent(Const.EVENT_NAME_SELECT_CUSTOMER)) // raise an Event with given name
        ?.withColValueAsEventDataAttr(MATRIX_COLUMN_CUSTOMER_ID, Const.EVENT_ATTRIBUTE_CUSTOMER_ID) // add data from the selected Matrix row to the Event's attributes
